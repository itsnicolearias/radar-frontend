...
interface SignalDetailModalProps {
  ...
  labels?: {
    now: string;
    agoMinutes: (count: number) => string;
    agoHours: (count: number) => string;
    yesterday: string;
    agoDays: (count: number) => string;
    respondSignal: string;
    pending: string;
    sendRequest: string;
    viewProfile: string;
    close: string;
...
export const SignalDetailModal: React.FC<SignalDetailModalProps> = ({
  ...
  const t = labels ?? {
    now: "Ahora",
    agoMinutes: (count: number) => `Hace ${count}m`,
    agoHours: (count: number) => `Hace ${count}h`,
    yesterday: "Ayer",
    agoDays: (count: number) => `Hace ${count} dias`,
    respondSignal: "Responder senal",
    pending: "Pendiente",
  ...
  const formatRelativeTime = (date: string | Date) => {
    ...
    if (diffMins < 60) return t.agoMinutes(diffMins);
    if (diffHours < 24) return t.agoHours(diffHours);
    if (diffDays === 1) return t.yesterday;
    return t.agoDays(diffDays);
  };
          ...
          <div className="relative mb-3">
            <div className="w-20 h-20 rounded-full bg-white flex items-center justify-center text-black font-bold text-2xl border-2 border-[#FF005C] overflow-hidden">
              {signal.Sender.Profile?.photoUrl ? (
                <img src={signal.Sender.Profile.photoUrl || "/placeholder.svg"} alt={signal.Sender.displayName || "U"} className="w-full h-full object-cover" />
              ) : (
